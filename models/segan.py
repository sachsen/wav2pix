...
class GBlock(nn.Module):
    def __init__(
        ...
        if enc:
            ...
            if convblock:
                self.conv = Conv1DResBlock(
                    ninputs, fmaps, kwidth, stride=pooling, bias=bias
                )
            else:
                self.conv = nn.Conv1d(
                    ninputs, fmaps, kwidth, stride=pooling, padding=padding, bias=bias
                )
            if snorm:
            ...
            if linterp:
                ...
                if activation == "glu":
                    self.glu_conv = nn.Conv1d(
                        ninputs, fmaps, kwidth, stride=1, padding=kwidth // 2, bias=bias
                    )
                    if snorm:
                        self.glu_conv = SpectralNorm(self.glu_conv)
            else:
                ...
                if activation == "glu":
                    # TODO: REVIEW
                    raise NotImplementedError
                    self.glu_conv = nn.ConvTranspose1d(
                        ninputs,
                        fmaps,
                        kwidth,
                        stride=pooling,
    ...
    def forward(self, x):
        ...
        if self.linterp:
            x = self.pre_conv(x)
            x = F.upsample(
                x, scale_factor=self.pooling, mode="linear", align_corners=True
            )
        if self.enc:
    ...
    def __init__(
        ...
        if enc:
            ...
            self.conv = nn.ConvTranspose2d(
                ninputs, fmaps, kwidth, stride=pooling, padding=padding
            )
        if bnorm:
    ...
    def __init__(
        ...
        if Genc is None:
            ...
            for d_i, d_fmap in enumerate(d_fmaps):
                ...
                if d_i == 0:
                    ...
                    inp = d_fmaps[d_i - 1]
                self.disc.append(DiscBlock(inp, kwidth, d_fmap, act, pooling=4))
        else:
        ...
        if pool_type == "none":
            ...
            if isinstance(act, nn.LeakyReLU):
                """
                Before feeding the audio to the FC layer module, it is scaled by 4.
                We will adapt the FC to the length of the wav.
                Example, if we work with 1s audios at 16000,
                we have initially 16000 samples and after the scaling,
                we would obtain 16000/4 = 4000 -> 4096.
                """
                input_dim = [
                    2**i for i in range(0, 15)
                ]  # defining powers of 2 until 16384,
                # which would fit for 4 audio seconds.
                input_dim.append(3072)  # THIS IS HARDCODED! We have seen we need this
                # input dimension when working with 0.7 s.
            ...
            pool_size = 128
            self.rnn = nn.LSTM(
                d_fmaps[-1], pool_size, batch_first=True, bidirectional=True
            )
            # bidirectional size
            pool_size *= 2
...
class Generator(Model):
    def __init__(
        ...
        if aal:
            ...
            from scipy.signal import cheby1, dimpulse, dlti
        ...
        for layer_idx, (fmaps, act) in enumerate(zip(enc_fmaps, activations)):
            if layer_idx == 0:
                inp = ninputs
            else:
    ...
    def forward(self, x, z=None):
        ...
        if z is None:
            ...
            z = Variable(
                torch.randn(hi.size(0), self.z_dim, *hi.size()[2:]).float()
            ).cuda()
        if len(z.size()) != len(hi.size()):
            raise ValueError(
                "len(z.size) {} != len(hi.size) {}"
