...
class generator(nn.Module):
    def __init__(self, image_size, audio_samples):
        ...
        self.audio_embedding = Discriminator(
            1, self.d_fmaps, 15, nn.LeakyReLU(0.3), self.audio_samples
        )
        ...
        if self.image_size == 64:
            self.netG = nn.Sequential(
                SpectralNorm(
                    nn.ConvTranspose2d(
                        self.latent_dim, self.ngf * 8, 4, 1, 0, bias=False
                    )
                ),
                nn.Dropout(),
        ...
        if self.image_size == 128:
            self.netG = nn.Sequential(
                SpectralNorm(
                    nn.ConvTranspose2d(
                        self.latent_dim, self.ngf * 16, 4, 1, 0, bias=False
                    )
                ),
                nn.Dropout(),
                nn.ReLU(True),
