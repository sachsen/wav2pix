...
parser.add_argument(
    "--dataset_path", required=True, help="path where the dataset is stored"
)
parser.add_argument(
    "--pickle_path", required=True, help="folder where to store the .pckl file"
)
...
face_file = open(
    os.path.join(args.pickle_path, "faces/" + args.save_name) + ".pkl", "wb"
)
audio_file = open(
    os.path.join(args.pickle_path, "audios/" + args.save_name) + ".pkl", "wb"
)
...
for youtuber in youtubers:
    ...
    faces = [
        os.path.join(working_path, face)
        for face in os.listdir(working_path)
        if face.endswith(".png")
    ]
    audios = [
        os.path.join(working_path, audio)
        for audio in os.listdir(working_path)
        if audio.endswith(".wav")
    ]

    # as there are some .wav files damaged in loics directory,
    # I should skip that audios and do not store them
    for face in faces:
        audio = (
            face.replace("cropped_face", "preprocessed")
            .replace(".png", ".wav")
            .replace(".jpg", ".wav")
        )
