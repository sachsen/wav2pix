...
class dataset_builder(Dataset):
    def __init__(self, transform=None, inference=False, audio_seconds=1):
        ...
        # selecting the paths for the images/audios we will work with according
        # to whether we are training or testing
        if not inference:
            ...
            print(
                ...
                ": {0} faces | {1} audios ".format(len(self.faces), len(self.audios))
            )
            print
        else:
            ...
            print(
                "Total amount of inference samples"
                ": {0} faces | {1} audios ".format(len(self.faces), len(self.audios))
            )
            print
        ...
        self.audio_samples = (
            audio_seconds * 16000
        )  # desired audio_samples to work with for each .wav file
        self.transform = transform
        self.inference = inference
        self.youtubers = list(
            set([path.split("/")[-2] for path in self.faces])
        )  # be carefull, this is hardcoded.
        # Each youtuber has its own directory with the images/audios inside
    ...
    def __getitem__(self, idx):
        ...
        # getting image paths. # in practice, when inference
        # that won't be a path but a 0
        ...
        if self.audio_samples > len(wav_data):
            raise ValueError(
                "Desired audio length is larger than the .wav file duration"
            )
        ...
        # storing youtuber identity in a onehot vector. It will be usefull,
        # for example, for softmax the loss computation
        youtuber = face_path.split("/")[-2]
        onehot = self.youtubers.index(youtuber)

        sample = {
            "onehot": self.to_categorical(onehot),
            "wrong_face": np.array(wrong_face),
            "face": np.array(cropped_face),
            "audio": wav_data,
    ...
    def format_filename(self, filename):
        try:
            ...
            s = "".join(
                (
                    c
                    for c in unicodedata.normalize("NFD", str(filename))
