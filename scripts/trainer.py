...
class Trainer(object):
    def __init__(
        ...
        # if pre_trained_disc is true, load the already learned parameters.
        # Else, initialize weights randomly
        if pre_trained_disc:
        ...
        self.dataset = dataset_builder(
            transform=Rescale(int(self.image_size)),
        ...
        # defining optimizers. Keeping all the parameters from the list of
        # Module.parameters() for which requires_grad is TRUE
    ...
    def train(self):
        ...
        for epoch in range(self.num_epochs):
            for sample in self.data_loader:
                ...
                # getting each key value of the sample in question (each sample
                # is a dictionary)
                ...
                id_labels = from_onehot_to_int(
                    onehot
                )  # list with the position of the youtuber which the
                # audio in question belongs
                ...
                # tensor of 64 (num of samples per batch) ones and
                # zeros that will be used to compute D loss.
                ...
                )  # so smooth_real_labels will now be 0.9
                ...
                # feeding D with the generated images and z vector
                # whose dimensions will be needed
                ...
                # feeding D with real images but not corresponding to
                # the wav under training
                outputs, _ = self.discriminator(wrong_images, z_vector)
                # computing D loss when feeding real images but not
                # the ones corresponding to the input audios
                ...
                # feeding D with the generated images and z vector.
                # Storing intermediate layer activations for loss computation purposes
                outputs, activation_fake = self.discriminator(fake_images, z_vector)

                # feeding D with the real images and z vector.
                # Storing intermediate layer activations for loss computation purposes
                ...
                # This is a customized loss function, the first term
                # is the mean square error loss
                # The second term is feature matching loss,
                # this measure the distance between the real and generated
                # images statistics by comparing intermediate layers activations
                # The third term is L1 distance between the generated and real images,
                # this is helpful for the conditional case
                # because it links the embedding feature vector directly to
                # certain pixel values.
                # ===========================================

                # computing first the part of the loss related to the softmax
                # classifier after the embedding
                ...
                )  # we have seen softmax_loss starts around 2 and
                # g_loss around 20... That's why we've scaled by 10
    ...
    def predict(self):
        ...
        # starting_id = (
        #     0  # this would be the lower_bound id for the next image to be stored
        # )
        ...
        for id, sample in enumerate(self.data_loader):
            ...
            txt = [
                self.dataset.youtubers[idx] + "_" + str(id)
                for idx, id in zip(token, ids)
            ]
            ...
            for image, t in zip(fake_images, txt):
                ...
                im.save(
                    "results/{0}/{1}.jpg".format(
                        self.save_path, t.replace("/", "")[:100]
                    )
                )
