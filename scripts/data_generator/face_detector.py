...
parser.add_argument(
    "-dataset_path", required=True, help="Parent directory where the dataset is stored"
)
parser.add_argument(
    "-confidence_thresh",
    required=True,
...
parser.add_argument(
    "-haarcascade_path", required=True, help="Path of the haarcascade xml file"
)
...
def check_audio_frames(faces_frames_path, audio_frames_path):
    """Function that checks if audio cropping has done correctly and,
    if not, it corrects it."""

    print("Checking audio and video crops sync...")
    faces_frames_ids = [
        int(x.split("_")[-1].split(".")[0]) for x in os.listdir(faces_frames_path)
    ]
...
for index, path in enumerate(youtubers_path):
    ...
    list_videos = [
        ...
        if os.path.isfile(os.path.join(videos_path, filename))
    ]
    list_audios = [
        filename
        for filename in sorted(os.listdir(audios_path))
        if os.path.isfile(os.path.join(audios_path, filename))
        and not filename.endswith("m4a")
    ]
    for video, audio in zip(list_videos, list_audios):

        # Define storage directories
        cropped_audio_path = os.path.join(
            audios_path, audio.replace("_preprocessed.wav", "_frames")
        )
        cropped_faces_path = os.path.join(
            videos_path, video.replace(".mp4", "_cropped_frames")
        )
        frames_faces_path = os.path.join(
            videos_path, video.replace(".mp4", "_full_frames")
        )
        ...
        print(
            "Processing videos from Youtuber "
            + str(index + 1)
            + " out of "
            + str(num_youtubers)
        )
        ...
        print(
            "Video Metadata: width --> {}, height --> {}, Average FPS --> {},"
        ...
        if (
            len(
                [
                    x
                    for x in os.listdir(frames_faces_path)
                    if os.path.isfile(os.path.join(frames_faces_path, x))
                ]
            )
            == 0
        ):
            ...
            try:
                for idx, frame in tqdm(enumerate(video_capture)):
                    ...
                    if idx % avg_fps == 0:
                        ...
                        audio_writer = wave.open(
                            os.path.join(
                                cropped_audio_path,
                                audio.replace(".wav", "_frame_" + str(idx) + ".wav"),
                            ),
                            "w",
                        )
                        ...
                        try:
                            max_confidence = np.argmax(
                                weights
                            )  # Take only the bbox with max confidence
                        ...
                        try:
                            ...
                            for i, (x, y, w, h) in enumerate([faces]):
                                ...
                                cropped_face = frame[
                                    y : y + h, x : x + w
                                ]  # Make bbox bigger to ensure it crops the whole face

                                cv2.imwrite(
                                    os.path.join(
                                        cropped_faces_path,
                                        "cropped_face_frame_" + str(idx) + ".jpg",
                                    ),
                                    cropped_face,
                                )
                                cv2.imwrite(
                                    os.path.join(
                                        frames_faces_path, "frame_" + str(idx) + ".jpg"
                                    ),
                                    frame,
                                )
