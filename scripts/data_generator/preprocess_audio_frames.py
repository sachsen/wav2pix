...
youtubers_dataset = [
    os.path.join(args.dataset_path, youtuber, "audio") for youtuber in youtubers
]
...
for youtuber_audio_path in youtubers_dataset:
    ...
    for aac_audio_fname in audio_files:
        os.rename(
            os.path.join(youtuber_audio_path, aac_audio_fname),
            os.path.join(
                youtuber_audio_path,
                format_filename(
                    aac_audio_fname.replace(" ", "")
                    .replace("'", "")
                    .replace('"', "")
                    .replace("(", "")
                    .replace(")", "")
        ...
        aac_audio_fname = format_filename(
            aac_audio_fname.replace(" ", "")
            .replace("'", "")
            .replace('"', "")
            .replace("(", "")
        ...
        if not os.path.exists(os.path.join(youtuber_audio_path, wav_audio_fname)):
            errors = aac2wav(
                os.path.join(youtuber_audio_path, aac_audio_fname),
                os.path.join(youtuber_audio_path, wav_audio_fname),
                errors,
            )
        ...
        if not os.path.exists(os.path.join(youtuber_audio_path, output_file)):
            ...
            audio_reader = wave.open(
                os.path.join(youtuber_audio_path, wav_audio_fname), "r"
            )
            audio_writer = wave.open(
                os.path.join(youtuber_audio_path, output_file), "w"
            )
            (
                nchannels,
                sampwidth,
