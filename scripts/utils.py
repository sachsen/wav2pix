...
class Utils(object):
    ...
    def weights_init(m):
        ...
        ):  # find returns ndex if found and -1 otherwise.
...
class Logger(object):
    ...
    def log_iteration_gan(
        self, epoch, d_loss, g_loss, real_score, fake_score, wrong_score
    ):
        print(
            "Epoch: %d, d_loss= %f, g_loss= %f, real score D(X)= %f, fake score D(G(X))= %f, wrong score: %f"
            % (
                epoch,
        ...
        self.logger.info(
            "Epoch: %d, d_loss= %f, g_loss= %f, D(X)= %f, D(G(X))= %f, wrong score: %f"
            % (
                epoch,
                d_loss.data.cpu().mean(),
                g_loss.data.cpu().mean(),
    ...
    def draw(self, right_images, fake_images):
        self.viz.draw(
            "generated images", fake_images.data.cpu().numpy()[:64] * 128 + 128
        )
