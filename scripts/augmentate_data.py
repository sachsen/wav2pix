...
def augment_data(wav_data, factor, audio_seconds):
    ...
    for id in range(0, factor):
        # selecting a center randomly but taking into consideration
        # there needs to be data enough "in the left" and "in the right"
        samples_to_retain = audio_seconds * 16000
        valid_centers = range(0, len(wav_data))[
            samples_to_retain / 2 : -samples_to_retain / 2
        ]
        ...
        audios.append(
            wav_data[center - samples_to_retain / 2 : center + samples_to_retain / 2]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--folder_path",
        required=True,
        help="path of the folder where to take the data and augment",
    )
    parser.add_argument("--factor", required=True, help="data augmentation factor")
    parser.add_argument(
        "--output_folder",
        required=True,
        help="folder where to store the augmented data",
    )
    parser.add_argument(
        "--audio_seconds",
        required=True,
        help="audio of the resulting augmented .wav files",
    )
    ...
    youtubers = [
        youtuber
        for youtuber in os.listdir(args.folder_path)
        if not youtuber.startswith(".")
    ]
    ...
    for youtuber in youtubers:
        ...
        for audio in audios:
            ...
            corresponding_face = os.path.join(
                working_path, "cropped_face_frame_" + number + ".png"
            )
            ...
            if len(wav_data) < 16000 * int(args.audio_seconds):
                raise ValueError(
                    "The original audio is shorter than the desired output"
                )

            # obtaining the audios as a result of the data augmentation
            wav_vectors = augment_data(
                wav_data, int(args.factor), int(args.audio_seconds)
            )
            ...
            for wav_vec in wav_vectors:
                ...
                wav_file = wavfile.write(
                    filename=os.path.join(
                        new_path, "preprocessed_frame_" + str(id_number) + ".wav"
                    ),
                    rate=16000,
                    data=wav_vec,
                )
                new_face_path = os.path.join(
                    new_path, "cropped_face_frame_" + str(id_number) + ".png"
                )
